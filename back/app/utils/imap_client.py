...
class IMAPClientPersonal:
    def __init__(self, host: str, port: int, user: str, password: str, use_ssl: bool = True):
        ...
        self.port = port
        self.user = user
        # Автоматически дешифруем пароль если он зашифрован
        ...
        try:
            ...
            for folder in folders:
                try:
                    ...
                    # Парсим строку формата: (\HasNoChildren) "/" "INBOX"
                    # или: (\HasNoChildren) "/" INBOX
                    # Ищем последнюю часть после разделителя
                    ...
                    # Пропускаем пустые и служебные папки
            ...
            # Если нет папок, возвращаем хотя бы INBOX
            if not folder_list:
                logger.warning("No folders found, returning default INBOX")
                folder_list = ['INBOX']

            # Сортируем папки в правильном порядке
    ...
    def get_messages(
        ...
        try:
            ...
            message_ids.reverse()  # Новые письма первыми
