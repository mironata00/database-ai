...
import { Search, Users, Bell, Upload, LogOut, UserCog } from 'lucide-react'
...
export default function HomePage() {
  ...
  const filteredSuppliers = suppliers.filter(supplier => {
    ...
    return supplier.name.toLowerCase().includes(query) || supplier.inn.includes(query) || (supplier.tags_array && supplier.tags_array.some(tag => tag.toLowerCase().includes(query)))
  })
    ...
    <div className="flex h-screen bg-gray-50">
      <aside className="w-64 bg-slate-900 text-white flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          <button onClick={() => alert('Функция в разработке')} className="w-full flex items-center justify-between px-4 py-3 rounded-lg hover:bg-slate-800 text-slate-300">
            <div className="flex items-center space-x-3">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b px-6 py-4">
          <div className="flex items-center space-x-4">
            <div className="flex-1">
              <input type="text" placeholder="Поиск по тегу, бренду или названию (напр. 'Knauf')" className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
        ...
        <div className="flex-1 overflow-auto p-8">
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-500">Поставщики не найдены</div>
          ) : (
